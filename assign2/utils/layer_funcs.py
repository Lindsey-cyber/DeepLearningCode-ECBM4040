...
def conv2d_forward(x, w, b, pad, stride):
    ...
    :return: 
        A 4-D array. Should have size (batch, new_height, new_width, num_of_filters).
    ...
    x_batch, height, width, channels = x.shape
    filter_height, filter_width, channels, num_of_filters = w.shape
    x_padded = np.pad(x, ((0,0),(pad,pad),(pad,pad),(0,0)),mode = 'constant', constant_values = 0)
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    out = np.zeros((x_batch, new_height, new_width, num_of_filters))
    for i in range(x_batch):
        for k in range(new_height):
            for l in range(new_width):
                for j in range(num_of_filters):
                    height_begin = k * stride
                    height_end = height_begin + filter_height
                    width_begin = l * stride
                    width_end = width_begin + filter_width
                    x_region = x_padded[i, height_begin : height_end, width_begin : width_end, :]
                    out[i, k, l, j] = np.sum(x_region * w[:, :, :, j]) + b[j]
    return out
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    :return: 
        (d_w, d_b), i.e. the derivative with respect to w and b. For example, d_w means how a change of each value
...
def max_pool_forward(x, pool_size, stride):
    ...
    out = np.zeros((x_batch, new_height, new_width, channels))
    for i in range(x_batch):
        for k in range(new_height):
            for l in range(new_width):
                for j in range(channels):
                    height_begin = k * stride
                    height_end = height_begin + pool_size
                    width_begin = l * stride
                    width_end = width_begin + pool_size
                    x_region = x[i, height_begin : height_end, width_begin : width_end, j]
                    out[i, k, l, j] = np.max(x_region)
    return out
